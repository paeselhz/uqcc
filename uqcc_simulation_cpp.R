...
for( n_ite_norig in 1:length(n_orig)){
  ...
  for( alfas_utili in 1:length(alfa_h1)){
    ...
    # for(repl in 1:2){
      ...
      v_bn_calc_acf <- compute_v_bn_cpp(mdh0_acf)
      
      bn_new_pad_acf <- compute_bn_pad_cpp(md_h0 = mdh0_acf, md_h1=mdh1_acf, pad=sqrt(v_bn_calc_acf))
      ...
      v_bn_calc_drift<-compute_v_bn_cpp(mdserh0_drift)
      
      bn_new_pad_drift <- compute_bn_pad_cpp(md_h0=mdserh0_drift, md_h1 = mdserh1_drift, pad=sqrt(v_bn_calc_drift))
